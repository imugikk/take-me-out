...
class Manage extends Core_Controller
{
	function __construct()
	{
		...
		$this->load->model('kota/Kota_model');
	...
	public function index()
	{
		...
		$data = array();
		$data['content'] = 'manage';
		$data['kota_asal'] = $this->Kota_model->list();
		$data['kota_tujuan'] = $this->Kota_model->list();
	...
	function page($pg = 1)
	{
		...
		if ($filter['kota_asal'] and $filter['kota_tujuan']) {
			...
			$paging['current'] 		= $pg;
			$paging['load_func_name'] = 'page_train';
	...
	function detail_list()
	{
		...
		$pesanan['total'] = $this->Pesanan_model->updateSummary($id);
		$this->load->view($data['content'], $data);
	}
